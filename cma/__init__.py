...
def typicalGetSimple(url, apiKey, authToken, environment=None):
    ...
    '''
    header = createHeader(authToken, apiKey)
    if environment:
...
def getSingleContentType(apiKey, token, region, contentType):
    '''
...
def getAllEntries(apiKey, contentType, token, region):
    '''
    ...
    '''
    url = '{region}v3/content_types/{contentType}/entries?include=count=true'.format(region=region, contentType=contentType)
    return typicalGetIterate(url, apiKey, token, 'entries', None)
...
def getAllExtensions(apiKey, token, region, query=None):
    '''
    Gets all extensions
    ...
    if query:
        url = url + '&query=' + query # query={"$and":[{"type": "field"},{"title": "JSON Editor"}]}
...
def getAllLanguages(apiKey, token, region, query=None):
    '''
    Gets all languages
...
def updateLabel(apiKey, authToken, uid, body, region):
    '''
    Updates a label
    sample url: https://api.contentstack.io/v3/labels/{label_uid}
    '''
    url = '{}v3/labels/{}'.format(region, uid)
    return typicalUpdate(apiKey, authToken, body, url, 'label')
...
def generateAssetPermanentURL(apiKey, token, region, assetUid, body):
    '''
    Creating a permanent URL for an asset (Only possible once for each asset)
    sample url: https://api.contentstack.io/v3/assets/your_asset_uid
    '''
    url = '{}v3/assets/{}'.format(region, assetUid)
    return typicalUpdate(apiKey, token, {'asset': body}, url, 'asset')
...
def publishEntry(apiKey, token, body, region, contentType, language, uid, environments, retry=False):
    '''
    Publishing an entry
    sample url: https://api.contentstack.io/v3/content_types/{content_type_uid}/entries/{entry_uid}/publish
    '''
    url = '{}v3/content_types/{}/entries/{}/publish'.format(region, contentType, uid)
    body = {
        "entry": {
    ...
    res = requests.post(url, headers=createHeader(token, apiKey), json=body)
    if res.status_code in (200, 201):
        return res.json()
    elif (res.status_code == 429) and not retry:
        config.logging.warning('{}We are getting rate limited. Retrying in 2 seconds.{}'.format(config.YELLOW, config.END))
        sleep(2) # We'll retry once in a second if we're getting rate limited.
        return publishEntry(apiKey, token, body, url, region, contentType, language, uid, environments, True)
    return None